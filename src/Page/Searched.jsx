...
function Searched() {
  ...
  const getSearched = async (name) => {
    try {
      ...
      const response = await fetch(`https://api.spoonacular.com/recipes/complexSearch?apiKey=${import.meta.env.VITE_KEY}&query=${name}&number=20&includeIngredients=${name}&cuisine=${name}`);  // Fetch recipes based on search query
